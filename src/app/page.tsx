...
export default function DashboardPage() {
  ...
  const goals: BarItem[] = [...seed.icp.goals]
    ...
    .map((g) => ({
      ...
      value: g.convRate ?? 0,
      right: `${g.convRate ?? 0}%`,
      sub: `${g.count} ppl · ${g.avgLtv != null ? `${usd(g.avgLtv)} LTV` : "—"}`,
      muted: g.smallSample,
    }));
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-zinc-900">Conversion Dashboard</h1>
        <p className="mt-1 max-w-2xl text-sm text-zinc-500">
          ...
          LinkedIn audience and ad in this app. Built from {num(seed.meta.totalRecords)} enriched records via the AI Central CRM
          pipeline.
        </p>
      </div>
      ...
      <Callout tone="amber" title="The economics that shape this campaign">
        Average LTV is <strong>{usd(c.avgLtv, { cents: true })}</strong> — a low-ticket product on a high-CPM platform. Keep CPA
        below LTV by leaning LinkedIn spend toward the <strong>yearly &amp; lifetime offers</strong> and the highest-value markets,
        and excluding the lowest-LTV regions. The <Link href="/audiences" className="font-medium text-amber-900 underline">audience designs</Link> do exactly that.
      </Callout>

      <div className="grid gap-4 lg:grid-cols-3">
        <Card title="New customers over time" subtitle="Monthly converters, Nov 2023 – Jun 2026" className="lg:col-span-2">
        ...
        <Card title="Top markets" subtitle="Share of converters · avg LTV" right={<Link href="/icp" className="text-xs font-medium text-indigo-600">Explore →</Link>}>
      ...
      <Card title="What buyers want — and what converts" subtitle="Self-reported goal, ranked by conversion rate">
        ...
        <p className="mt-4 border-t border-zinc-100 pt-3 text-xs text-zinc-500">
          ...
          become the <Link href="/brief" className="font-medium text-indigo-600 underline">ad angles</Link>.
        </p>
      </Card>
    </div>
  );
